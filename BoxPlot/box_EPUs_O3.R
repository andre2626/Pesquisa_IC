...
##### Insolaçao  #####
STempbox$Insolacao
boxInsol <-plot_ly(data=STempbox, y= ~Insolacao, x = ~periodo, name = "Insol", type="box")
boxInsol <-boxInsol %>% layout(yaxis=list(title="Insol (horas)"),xaxis=list(label=c("Antes","Pico","Depois")))
boxInsol <-boxInsol %>% layout(xaxis=list(title=""),font=list(size=23))
boxInsol

##### Irradiação  #####
STempbox$Irradiacao
boxIrrad <-plot_ly(data=STempbox, y= ~Irradiacao, x = ~periodo, name = "Irradiacao", type="box")
