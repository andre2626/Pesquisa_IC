...
met[,14]<-(STemp$periodo) 
colnames(met)[14]="periodo"
...
dados<-dados[!is.na(dados[,14]),]
dados$periodo<-factor(dados$periodo,levels =c("Antes","Pico","Depois")) 


dados %>% select(periodo) %>% filter(periodo =="Depois") %>% nrow() #para saber numero de linhas "Depois"
dados<-dados %>% filter(!periodo =="Depois") # excluir linhas em que dados$periodo =="Depois" (excluir linha com Depois)
...
predict(modelo3)
?predict
saida<-predict(modelo3, newdata=x_test[,c(3,4,11,12)], type='response')
...
eventoprevisto <- as.factor(ifelse(probab3>=0.488,"1","0"))
