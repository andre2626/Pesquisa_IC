...
#EPUlistmp10=read.csv("G:/Meu Drive/4 IC/1 Projeto/Reuniões/7 Organização e Dados + importantes/Base de DADOS/list EPUs O3.CSV",header=T)


#escolher Duracao (MP10 =5, O3 =3)
poluente <- "MP10"
duracao<-5
lin<-which(EPUlistmp10$Duração_do_EPU==duracao) #linhas com duracao selecionada
...
for (j in c(3:5,7:20)) { # loop pelas variáveis de STemp
  ...
  for (i in 1:length(lin)) { # loop pelos eventos de interesse
    inicio<-which(STemp$data==EPUlistmp10$Início_do_EPU[lin[i]])
    fim<-which(STemp$data==EPUlistmp10$Término_do_EPU[lin[i]])
